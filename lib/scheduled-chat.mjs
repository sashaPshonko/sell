...
function scheduleUpsellRepeat(state, chatId, orderId, { fromSentAtMs } = {}) {
    ...
    const base = Number.isFinite(fromSentAtMs)
        ? fromSentAtMs
        : Date.parse(order?.profileUpsellSentAt || '') || Date.now();
...
export function restorePendingUpsellRepeats(state) {
    for (const order of Object.values(state.orders || {})) {
        if (!order?.profileUpsellSentAt || order.profileUpsellRepeatSentAt) continue;
        ...
        scheduleUpsellRepeat(state, chatId, orderId, {
            fromSentAtMs: Date.parse(order.profileUpsellSentAt),
        });
    }
}
    ...
    if (item.kind === 'profile_upsell_repeat') {
        ...
        if (cached?.url) {
            const text = buildProfileUpsellRepeatHint({
                emoji: cached.emoji,
                upsellKk: cached.kk,
                priceRub: cached.priceRub ?? order?.itemPriceRub,
                url: cached.url,
            });
