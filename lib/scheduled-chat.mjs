...
function queueHasPopcornTrial(state, orderId) {
    return state.scheduledChatMessages?.some(
        (i) => i.kind === 'popcorn_trial' && i.orderId === orderId,
    );
}
...
function buyerAlreadyBoughtGiftLot(state, buyerId) {
...
/**
 * После выдачи валюты: либо 🎁 «выгоднее на профиле», либо ссылка на ботов.
 * Уже брал 🎁 — только боты. Иначе 50/50.
 * @param {{ roll?: number }} [opts] roll 0..1 для тестов
 */
export function pickPostDeliveryUpsellKind(state, order, opts = {}) {
    if (!order) return null;
    if (isSubscriptionLot(order.itemName, order.itemSlug, order.itemId)) return null;
    const hasPopcorn = Boolean(popcornTrialUrl());
    const gift =
        isMarkedProfileLot(order.itemName) ||
        buyerAlreadyBoughtGiftLot(state, order.buyerId);
    if (gift) return hasPopcorn ? 'popcorn' : null;
    if (!hasPopcorn) return 'profile';
    const roll = Number.isFinite(opts.roll) ? opts.roll : Math.random();
    return roll < 0.5 ? 'popcorn' : 'profile';
}

function schedulePopcornTrialMessage(state, chatId, orderId) {
    if (!popcornTrialUrl()) return;
    const order = getOrder(state, orderId);
    if (!order) return;
    if (order.popcornTrialSentAt || order.popcornTrialScheduledAt) return;
    if (queueHasPopcornTrial(state, orderId)) return;
    ...
    setOrderPhase(state, orderId, order.phase ?? 'completed', {
        popcornTrialScheduledAt: new Date().toISOString(),
        popcornTrialSendAtMs: sendAtMs,
        postDeliveryUpsellKind: 'popcorn',
    });
}

/** После выдачи — 🎁-апселл или ссылка на ботов (не оба). */
export function schedulePostDeliveryMessages(state, chatId, orderId) {
    const order = getOrder(state, orderId);
    if (!order) return;
    if (isSubscriptionLot(order.itemName, order.itemSlug, order.itemId)) return;

    const existing = order.postDeliveryUpsellKind;
    const kind =
        existing === 'popcorn' || existing === 'profile'
            ? existing
            : pickPostDeliveryUpsellKind(state, order);

    if (!kind) return;
    if (!existing) {
        setOrderPhase(state, orderId, order.phase ?? 'completed', {
            postDeliveryUpsellKind: kind,
        });
    }

    if (kind === 'popcorn') {
        schedulePopcornTrialMessage(state, chatId, orderId);
        return;
    }
    ...
    setOrderPhase(state, orderId, order.phase ?? 'completed', {
        ...
        postDeliveryUpsellKind: 'profile',
    });
}
...
async function buildScheduledPayload(client, state, item, order) {
    if (item.kind === 'popcorn_trial') {
        const text = buildPopcornTrialHint();
        return { text: text || null, drop: !text, match: null };
    }
...
function alreadySent(order, kind) {
    ...
    if (kind === 'popcorn_trial') return Boolean(order?.popcornTrialSentAt);
    return false;
}
...
function markSent(item, order, state, match) {
    ...
    if (item.kind === 'popcorn_trial' && orderNow) {
        setOrderPhase(state, orderId, orderNow.phase, {
            popcornTrialSentAt: new Date().toISOString(),
        });
    }
}
