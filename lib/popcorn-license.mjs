...
import {
    findAllSubscriptionPaidDeals,
    findBuyerEmailInChat,
} from '../parse.mjs';
...
function buildKeyChatText(keyCode, days, installerUrl, mailed) {
    const lines = [
    ...
    if (mailed) lines.push('📧 Дубликат ключа отправлен на почту.');
    else {
        lines.push('📧 Чтобы получить ключ на почту — напиши: /mail ты@почта.ru');
    }
    return lines.join('\n');
}

async function issueOnLicense({ dealId, days, email, note }) {
    ...
    const res = await fetch(`${licenseBase()}/v1/admin/issue-subscription`, {
        ...
        body: JSON.stringify({ dealId, days, email, note, adminKey: token }),
    });
...
export async function fulfillPopcornSubscriptions(client, state, chatId, messages) {
    ...
    for (const paid of deals) {
        ...
        if (rec.chatSent && rec.playerokSent && (rec.mailed || !email)) continue;
        ...
        const email =
            findBuyerEmailInChat(messages, paid.buyerId, paid.paidAt) ||
            paid.buyerEmail ||
            rec.email ||
            null;
        ...
        try {
            ...
            rec.mailed = Boolean(data.mailed);

            if (!rec.chatSent && keyCode) {
                await sendChatMessage(
                    client,
                    paid.chatId || chatId,
                    buildKeyChatText(keyCode, paid.days, data.installerUrl, rec.mailed),
                );
                rec.chatSent = true;
            } else if (email && data.mailed && !rec.mailTold) {
                await sendChatMessage(
                    client,
                    paid.chatId || chatId,
        ...
        rec.done = Boolean(rec.chatSent && rec.playerokSent);
            console.log(
                `[sell] popcorn ключ ${paid.dealId.slice(0, 8)}… ${paid.days}д mailed=${rec.mailed}`,
            );
        } catch (e) {
