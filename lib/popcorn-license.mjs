...
function popcornDriveUrl() {
    return String(
        process.env.POPCORN_DRIVE_URL ||
            'https://drive.google.com/drive/folders/1Z4Z8q3fEV5gNJ7Qjy32Q7D7zvAjvy-3W?usp=drive_link',
    ).trim();
}
...
function buildKeyChatText(keyCode, days) {
    const lines = [
        ...
        `📅 Срок: ${days} дн.`,
        '',
        'Скачай .exe с Диска и установи.',
        'Там же видео, как ставить.',
        'Потом открой программу и вставь ключ.',
    ];
    const drive = popcornDriveUrl();
    if (drive) lines.push('', `⬇️ Диск: ${drive}`);
    return lines.join('\n');
}
