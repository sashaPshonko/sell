...
/**
 * Самый большой 🎁-лот ≤ maxKk (без премки), на который хватает баланса бота.
 * Среди равных кк предпочитаем ту же/меньшую цену, что у текущего заказа.
 */
export async function resolveGiftLotUpToKk(
    ...
    for (const item of items) {
        ...
        if (orderPrice != null && listPrice != null && listPrice > orderPrice) continue;
    ...
    for (const c of candidates.slice(0, 12)) {
        const verified = await fetchListingPriceBySlug(client, c.item.slug);
        if (!verified) continue;
        if (orderPrice != null && verified.priceRub > orderPrice) continue;
        const url = productUrl(c.item.slug);
        if (!url) continue;
        return {
            url,
