...
import { isCurrencyKkLot, parseAmountKk, discountedPriceRub } from '../parse.mjs';
...
/**
 * Уже была успешная выдача этому buyerId (не считая текущего заказа и автовозвратов upsell).
 * Первая покупка → false: не делаем refund, выдаём + ссылку после delivery.
 */
export function buyerHasPriorSuccessfulDelivery(state, buyerId, excludeOrderId = null) {
    ...
    for (const o of Object.values(state.orders)) {
        ...
        if (excludeOrderId && o.orderId === excludeOrderId) continue;
        if (o.cancelReason === 'profile_upsell_refund') continue;
        if (o.gameDeliveryAt || o.buyerNotifiedAt || o.playerokMarkedAt) return true;
        if (o.phase === 'completed') return true;
    }
    return false;
}
...
function pickUpsellItem(items, order) {
    ...
    for (const item of items) {
        ...
        const itemPrice = discountedPriceRub(item);
