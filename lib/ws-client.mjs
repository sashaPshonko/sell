...
let connected = false;
const eventQueue = [];
/** После обрыва/рестарта sellbot — переотправить live dispatched/ws_pending */
let needsOrderResync = false;
...
function connect() {
    ...
    ws.on('open', () => {
        connected = true;
        needsOrderResync = true;
        // Старый кэш баланса с прошлой сессии sellbot невалиден, пока снова не придёт /balance
        eventQueue.push({ type: 'bot_balance', balance: null, reason: 'ws_reconnect' });
    ...
    ws.on('message', (raw) => {
        try {
            ...
            if (msg.type === 'bot_event' && msg.event) {
...
/** true один раз после connect/reconnect — пора re-push live заказов */
export function consumeOrderResync() {
    if (!needsOrderResync) return false;
    needsOrderResync = false;
    return true;
}
...
export function pushOrder(order) {
    ...
    if (!sendWhenReady(payload)) {
        ...
        console.log(`[ws-client] заказ ${order.orderId}${order.resync ? ' (resync)' : ''}`);
    }
