...
function itemMatchesOrder(item, order) {
    ...
    if (wantProfile) {
        // 🎁 в completed-list часто без 🎁 (PlayerOK показывает ·) — матчим по kk.
    } else {
...
async function resolveItemBySlug(client, slug) {
    ...
    try {
        ...
        // Уже в продаже без buyer — перевыставлять нечего.
        if (item.status === 'APPROVED' && !item.buyer?.id && !item.buyer?.username) {
