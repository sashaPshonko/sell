...
/**
 * Повторный /nick игнорируем ТОЛЬКО если заказ уже закрыт.
 * Пока заказ открыт — ник всегда принимаем (и mid-delivery тоже).
 */
...
/** Бан/капча/прокси — не крутить выдачу, пока бот не заменён (см. recoverBotFatalPausedOrders). */
export const BOT_FATAL_DELIVERY_ERRORS = new Set([
    'banned',
    'captcha',
    'proxy_timeout',
    'bot_offline',
...
export function isBotFatalDeliveryError(reason) {
    return BOT_FATAL_DELIVERY_ERRORS.has(String(reason || ''));
}

/** После notifyBotFatalToQueue — блок dispatch и «Сейчас выдаю» до восстановления бота. */
export function isBotFatalPaused(order) {
    if (!order) return false;
    return Boolean(order.botStatusHintAt) && isBotFatalDeliveryError(order.lastError);
}

/** Снять bot-fatal паузу (sellbot снова онлайн) — покупатель всё ещё шлёт /nick. */
export function recoverBotFatalPausedOrders(state, { username } = {}) {
    let n = 0;
    for (const o of Object.values(state?.orders || {})) {
        if (!isBotFatalPaused(o)) continue;
        const oid = o.orderId || o.dealId;
        if (!oid) continue;
        setOrderPhase(state, oid, o.phase || 'awaiting_nick', {
            botStatusHintAt: undefined,
            lastError: undefined,
        });
        n++;
    }
    if (n > 0) {
        console.log(
            `[sell] bot-fatal пауза снята на ${n} заказ(ах)` +
                (username ? ` (${username})` : ''),
        );
    }
    return n;
...
export function canDispatchToSellbot(order) {
    ...
    if (isBotFatalPaused(order)) return false;
