 ...
 * ws_pending сюда не входит: иначе canDispatch / retryWsPending сломается.
 */
export function shouldIgnoreNickRedispatch(order) {
    ...
    // Активная выдача у бота — не сбрасываем флаги и не делаем retry reset
    if (order.phase === 'dispatched') return true;
    if (order.clanWithdrawHintSentAt && order.phase === 'ws_pending') return true;
    return false;
}
