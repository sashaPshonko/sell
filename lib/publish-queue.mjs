...
function burstPauseMs() {
    // После 429 не тыкаем createItem каждые 45с — PlayerOK так не отпускает.
    return Number(process.env.PUBLISH_BURST_PAUSE_MS || 15 * 60 * 1000);
}

function rateLimitLotBackoffMs(attempt) {
    const base = Number(process.env.PUBLISH_RATE_LIMIT_LOT_MS || 15 * 60 * 1000);
    const cap = Number(process.env.PUBLISH_RATE_LIMIT_MAX_MS || 40 * 60 * 1000);
    const exp = Math.min(Math.max(0, Number(attempt) || 0), 2);
...
export function notePublishRateLimit(err) {
    ...
    console.warn(
        `[sell] publish-queue: 429 → пауза всей очереди ${Math.round(burstPauseMs() / 1000)}с ` +
            `(не долбим PlayerOK, пока не остынет)`,
    );
}
