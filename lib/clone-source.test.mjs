...
test('живой APPROVED без buyer — уже на витрине', () => {
    assert.equal(
        isLiveOnSale({ status: 'APPROVED', slug: 'x', buyer: null }),
        true,
    );
    assert.equal(
        isLiveOnSale({ status: 'SOLD', slug: 'x' }),
        false,
    );
    assert.equal(
        isLiveOnSale({ status: 'APPROVED', buyer: { id: 'u1' } }),
        false,
    );
});
